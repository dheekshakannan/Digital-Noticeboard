...
exports.seedDefaultAdmin = exports.getMe = exports.login = void 0;
...
const login = async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ id: user._id, role: user.role }, secret, { expiresIn: '1d' } // Token expires in 24 hours
        );
        res.status(200).json({
            success: true,
            token,
...
const seedDefaultAdmin = async () => {
    try {
        ...
        if (adminCount === 0) {
            ...
            const defaultAdmin = new User_1.User({
        ...
        if (studentCount === 0) {
            ...
            const salt = await bcryptjs_1.default.genSalt(10);
            const studentPasswordHash = await bcryptjs_1.default.hash(defaultStudentPassword, salt);
            const defaultStudent = new User_1.User({
